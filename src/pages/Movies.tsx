    ...
    <div>
      ...
      <div className="w-full max-w-screen-xl px-4 mx-auto">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 text-center md:text-left">
          <h1 className="text-2xl sm:text-3xl font-sans font-semibold text-[#00DF5E]">
        ...
        <AnimatePresence>
          {!loading && movies.length === 0 ? (
            <motion.div
              ...
              className="flex flex-col items-center justify-center px-4 py-16 sm:py-20 md:py-24 lg:py-28 bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#00DF5E]/10 rounded-2xl shadow-2xl mt-10 mb-10 border border-[#00DF5E]/30 text-center"
            >
              <svg className="w-16 h-16 sm:w-20 sm:h-20 mb-6 animate-bounce-slow" fill="none" stroke="#00DF5E" strokeWidth="1.5" viewBox="0 0 48 48">
              ...
              <span className="text-2xl sm:text-3xl font-bold text-[#00DF5E] mb-2 drop-shadow-lg">
                Ops! Nenhum filme encontrado
              </span>

              <span className="text-base sm:text-lg text-gray-300 mb-8 max-w-md sm:max-w-xl">
                Não conseguimos encontrar nenhum resultado para sua busca. Tente outro nome ou clique no botão abaixo para voltar e explorar outros filmes!
              </span>
              ...
              <motion.button
                ...
                className="mt-2 px-6 sm:px-8 py-3 bg-[#00DF5E] text-[#0f2027] rounded-full font-bold shadow-lg hover:bg-[#00b84c] transition-colors text-base sm:text-lg tracking-wide"
              >
                Voltar
              </motion.button>
            </motion.div>
        ...
        >
          {page > 5 && (
            <>
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                onClick={() => setPage(1)}
                className="text-base sm:text-lg md:text-xl font-semibold font-sans mr-2 px-2 sm:px-3 py-1 rounded hover:bg-[#00DF5E]/10 transition-colors"
              >
                first
              </motion.button>
              <motion.button
                ...
                className="text-base sm:text-lg md:text-xl font-semibold font-sans px-2 sm:px-3 py-1 rounded hover:bg-[#00DF5E]/10 transition-colors"
              >
                {'<'}
              </motion.button>
            </>
            ...
            return Array.from({ length: Math.min(5, totalPages) }, (_, i) => start + i)
              .map(num => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                  onClick={() => setPage(num)}
                  className={`text-base sm:text-lg md:text-xl font-semibold font-sans px-2 sm:px-3 py-1 rounded transition-colors ${
                    page === num ? 'underline text-[22px] sm:text-[26px] md:text-[28px] bg-[#00DF5E]/10' : ''
                  }`}
                >
                  {num}
                </motion.button>
          ...
          {totalPages > 1 && (
            <motion.button
              ...
              className="text-base sm:text-lg md:text-xl font-semibold ml-2 px-2 sm:px-3 py-1 rounded hover:bg-[#00DF5E]/10 transition-colors"
            >
              last
            </motion.button>
          )}
